...
cd /opt
test -d ./aports || git clone --depth 1 --branch v3.18.0 git://git.alpinelinux.org/aports
cd /opt/aports/main/strace

# libunwind (1.6.2-r3)
git apply --recount "$script_pwd/strace603libunwind.diff"
build
tar -xzvf /root/packages/main/x86_64/strace-6.3-r0.apk
cp ./usr/bin/strace /share/static/strace603libunwind
